...
int FlexCAN::blockWrite (const CAN_message_t &msg)
{
    uint32_t index;
    ...
    while(buffer <= -1){ //keep trying to put the CAN msg in the mailbox until there is room and it succeeds
      for (index = NUM_MAILBOXES - numTxMailboxes - 1; index < NUM_MAILBOXES; index++) { //for each mailbox
        //Serial.print("Looking at Mailbox: ");
        ...
        if (buffer > -1) {
          dbg_println ("Writing a frame directly.");
          //Serial.printf("Writing CAN frame containing ID %04X\n",msg.id & 0x7FF);

          blockWriteTxRegisters (msg, buffer);
          return 1;
      }
      //continue the loop if no mailbox, should force each message to call blockWriteTxRegisters
      //this method is now blocking
    }
...
void FlexCAN::blockWriteTxRegisters (const CAN_message_t &msg, uint8_t buffer)
{
    // transmit the frame

    FLEXCANb_MBn_CS(flexcanBase, buffer) = FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_INACTIVE);

    /*while (FLEXCANb_MBn_CS(flexcanBase, buffer) != FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_INACTIVE)){
      //busy waits until the CS register is updated
      Serial.println("Updating");
    }*/

    if (msg.flags.extended) {
        FLEXCANb_MBn_ID(flexcanBase, buffer) = (msg.id & FLEXCAN_MB_ID_EXT_MASK);
       /* while (FLEXCANb_MBn_ID(flexcanBase, buffer) != (msg.id & FLEXCAN_MB_ID_EXT_MASK)){
      //busy waits until the ID register is updated
          Serial.println("Updating");
      }*/
    } else {
        FLEXCANb_MBn_ID(flexcanBase, buffer) = FLEXCAN_MB_ID_IDSTD(msg.id);
        /*while (FLEXCANb_MBn_ID(flexcanBase, buffer) != FLEXCAN_MB_ID_IDSTD(msg.id)){
      //busy waits until the ID register is updated
          Serial.println("Updating");
      }*/
    }
    ...
    FLEXCANb_MBn_WORD0(flexcanBase, buffer) = (msg.buf[0]<<24)|(msg.buf[1]<<16)|(msg.buf[2]<<8)|msg.buf[3];
    /*while (FLEXCANb_MBn_WORD0(flexcanBase, buffer) != (msg.buf[0]<<24)|(msg.buf[1]<<16)|(msg.buf[2]<<8)|msg.buf[3]){
      //busy waits until the WORD0 register is updated
      Serial.println("Updating");
    }*/

    FLEXCANb_MBn_WORD1(flexcanBase, buffer) = (msg.buf[4]<<24)|(msg.buf[5]<<16)|(msg.buf[6]<<8)|msg.buf[7];
   /* while (FLEXCANb_MBn_WORD1(flexcanBase, buffer) != (msg.buf[4]<<24)|(msg.buf[5]<<16)|(msg.buf[6]<<8)|msg.buf[7]){
      //busy waits until the WORD1 register is updated
      Serial.println("Updating");
    }*/
    ...
    if (msg.flags.extended) {
        if (msg.flags.remote) {
            ...
            /*while (FLEXCANb_MBn_CS(flexcanBase, buffer) != FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_ONCE) |
                                                   FLEXCAN_MB_CS_LENGTH(msg.len) | FLEXCAN_MB_CS_SRR | 
                                                   FLEXCAN_MB_CS_IDE | FLEXCAN_MB_CS_RTR){
          //busy waits until the CS register is updated
          Serial.println("Updating");
